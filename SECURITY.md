...
Applied today, the activity test classifies every `1.x` line from `1.0` to `1.6` as supported, because each has had a patch release within six months. That is a wide set, and it is deliberate rather than accidental — the project cuts minors frequently and adopters do not all move at that pace. It does not mean every line receives every fix. Backport effort is prioritised by severity, by whether the affected component is reachable in a default configuration, and by how many known adopters are on the line, so a critical fix reaches every supported line while a moderate one may land only on the most recent. The lower rows of the table are therefore forward-looking today: no `1.x` line has yet gone six months without a patch.
